...
func _ready() -> void:
	...
	if Global.blue_team_genders == 0:
		var texture = load("res://Sprites/PlayerFemale.png")
		$Sprite3D.texture = texture
	else:
		var texture = load("res://Sprites/PlayerMaleReceiver.png")
		$Sprite3D.texture = texture
...
func _physics_process(_delta: float) -> void:
	...
	if Global.point_won == false:
		if Global.receive_pause == false and Global.spike_pause == false:
			...
			if Global.player_status == 0:
				controlling = true
				if Global.tutorial == false or Global.tutorial and Global.served or Global.tutorial and Global.initial_tutorial:
